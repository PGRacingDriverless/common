...
ConePairArray::ConePairArray(const common_msgs::msg::ConePairArray::SharedPtr msg)
{
    ...
    for (const auto &msg_cone_pair : msg->cone_pair_array)
    {
        ...
        cone_pair.setLeft(Cone(msg_cone_pair.cone_left.x, msg_cone_pair.cone_left.y));
        cone_pair.setRight(Cone(msg_cone_pair.cone_right.x, msg_cone_pair.cone_right.y));
...
{
    ConeArray left_cone_array;
    ConeArray right_cone_array;
    for (auto &item : *this)
    {
